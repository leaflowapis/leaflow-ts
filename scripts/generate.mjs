...
//
//   openapi/<service>/<subpackage>/<version>/openapi.yaml
//       → gen/<service>/<subpackage>/<version>/...   the same three files
//       → gen/<service>/index.ts                     one namespace per subpackage
//
...
//
// type/ holds shared shapes rather than a service, and has no openapi.yaml at any depth.
function findContracts(directory, prefix) {
  return readdirSync(directory, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .flatMap((entry) => {
      const spec = join(directory, entry.name, "openapi.yaml");
      if (prefix.length > 0 && existsSync(spec)) {
        return [{ service: prefix.join("/"), version: entry.name, spec }];
      }
      const path = [...prefix, entry.name];
      return path.length < 3 ? findContracts(join(directory, entry.name), path) : [];
    });
}

const contractList = findContracts(contracts, []).sort(
  (a, b) => a.service.localeCompare(b.service) || a.version.localeCompare(b.version),
);
  ...
  // somewhere else, which no type checker can catch.
  {
    writeFileSync(
      ...
      `${[
        ...
        "",
        `/** A client for the ${service.replace("/", " ")} service. Pass baseUrl to override the address. */`,
        server
...
// A service split by function gets one namespace per subpackage inside its own namespace:
// billing.catalog, billing.account. A joined name (billingCatalog) would read as a separate
// service, and the nesting is what the package path and the Go import path already say.
const barrel = [];
const subpackages = new Map();
for (const { service, version } of services) {
  const [head, ...rest] = service.split("/");
  if (rest.length === 0) {
    barrel.push(`export * as ${head} from "./${service}/${version}/index.js";`);
    continue;
  }
  if (!subpackages.has(head)) {
    subpackages.set(head, []);
    barrel.push(`export * as ${head} from "./${head}/index.js";`);
  }
  subpackages
    .get(head)
    .push(`export * as ${rest.join("_")} from "./${rest.join("/")}/${version}/index.js";`);
}
for (const [head, lines] of subpackages) {
  if (services.some(({ service }) => service === head)) {
    console.error(
      `${head} has both its own contract and subpackages; one namespace cannot hold both`,
    );
    process.exit(1);
  }
  writeFileSync(
    join(output, head, "index.ts"),
    `${[
      "// Code generated by scripts/generate.mjs. DO NOT EDIT.",
      "//",
      `// ${head} is split by function; each subpackage is its own contract and namespace.`,
      "",
      ...lines,
    ].join("\n")}\n`,
  );
